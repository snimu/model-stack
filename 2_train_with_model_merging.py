...
class GPT(nn.Module):
    ...
    def __init__(self, config: GPTConfig):
        ...
        if config.from_model is not None:
            ...
            loaded = torch.load(from_model)["model"]
            loaded = {k.replace("_orig_mod.", ""): v for k, v in loaded.items()}

            self.lm_head.weight.data = loaded["lm_head.weight"]
            if not config.only_load_lm_head:
...
@torch.no_grad()
def merge_models(models: list[GPT], merge_weights: list[float]) -> GPT:
    """Merge the transformer layers of two models."""
    assert len(models) == len(merge_weights)
    merge_weights = torch.tensor(merge_weights, dtype=torch.float32)
...
def eval_stack(
    ...
    model = ModelStack(
        models,
        use_first_layer=use_first_layer,
    ...
    if master_process:
        ...
        results = dict(
            ...
            num_tokens_seen=[int(8*64*1024*num_iterations)],  # batch_size*sequence_length*num_iterations
        ...
        print(df)
        if os.path.exists(f"{savefile}.csv"):
            with open(f"{savefile}.csv", "ab") as f:
                df.write_csv(f, include_header=False)
        else:
            df.write_csv(f"{savefile}.csv")
...
def main():
    ...
    parser.add_argument(
        '--mixin-weight', type=float, default=0.0,
        help="Weight of mixin model. type=float, default=0.0",
    )
    parser.add_argument(
        '--mixin-every', type=int, default=0,
        help="Mixin every n steps. type=int, default=0",
    )
    parser.add_argument(
        '--mixin-from', type=str, default=None,
        help="Mixin from this model. type=str, default=None",
    )
